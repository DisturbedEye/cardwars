...
{
public:
	explicit Deck(ACollection *col, int card_count_by_width, const float &height);
...
inline engine::Deck::Deck(ACollection *col, int card_count_by_width, const float &height)
	: position(0, 0), collection(col)
{
	cs = collection->getCardSize();
	setSize(card_count_by_width, Vec2f(0, 0), height);
}
