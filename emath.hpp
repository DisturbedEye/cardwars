...
namespace engine
{
	...
	template<class T>
	size_t math::get_index(const std::vector<T> &v, const T n)
	{
	...
	inline float math::around(const float &x, const int n) // округление до n-ого чисела после запятой
	{
		return std::roundf(x * powf(10, static_cast<float>(n))) / powf(10, static_cast<float>(n));
	}

	inline bool math::belongs(const float &x, const float &m, const float &n)
	{
		return x <= m and x >= n or x <= n and x >= m;
	}

	inline float math::time()
	{
		return static_cast<float>(clock()) / CLOCKS_PER_SEC;
	}

	inline bool math::contains(const Rect &rect, const Vec2f &p)
	{
		return rect.getGlobalBounds().contains(p);
	}

	inline bool math::contains(const Vec2f &ro, const Vec2f &rs, const Vec2f &p)
	{ // ro - across left top point
		return sf::FloatRect(ro, rs).contains(p);
	}

	inline bool math::contains(const Convex &polygon, const Vec2f &p)
	{ // only for convex
		bool ins = true;
	...
	inline bool math::contains(const Circle &circle, const Vec2f &point)
	{
	...
	inline float math::length(const Vec2f &v)
	{
		return hypotf(v.x, v.y);
	}

	inline float math::length(const float& x, const float& y)
	{
		return hypotf(x, y);
	}

	inline sf::Color math::mix(const sf::Color &c1, const sf::Color &c2)
	{
		return sf::Color((c1.r + c2.r) / 2, (c1.g + c2.g) / 2, (c1.b + c2.b) / 2, (c1.a + c2.a) / 2);
	}

	inline float math::clamp(const float &x, const float &minX, const float maxX)
	{
