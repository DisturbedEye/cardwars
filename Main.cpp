...
int main_menu(sf::RenderWindow &window)
{
	...
	Vec2f bsize = Vec2f(res.x / 8, res.y / 18);
	// buttons
	auto char_size = (uint32_t)abs(floor(engine::math::length(res))) / 54u;
	Button start(bsize,"Start", font, char_size);
	Button settings(bsize, "Settings", font, char_size);
	Button about_us(bsize, "About Us", font, char_size);
	Button exit(bsize, "Exit", font, char_size);
	...
	while (window.isOpen())
	{
		float time = emath::time(); // current time after start program
		sf::Event event;
		while (window.pollEvent(event))
		{
			switch (event.type)
